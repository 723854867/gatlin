...
public class AccountDetail implements Serializable {
	...
	public List<LogAccount> platformLogs() {
		...
		platformTips.forEach(tips -> {
			...
			log.setFieldType(tips.field);
			log.setAccountType(tips.type);
			log.setId(IDWorker.INSTANCE.nextSid());
			log.setOwnerType(TargetType.PLATFORM);
	...
	public List<LogAccount> userLogs() {
		...
		userTips.entrySet().forEach(entry -> {
			entry.getValue().forEach(tips -> {
				...
				log.setAccountType(tips.type);
				log.setId(IDWorker.INSTANCE.nextSid());
				log.setOwnerType(TargetType.USER);
				String bizId = StringUtil.hasText(tips.bizId) ? tips.bizId : this.bizId;
				log.setBizId(StringUtil.hasText(bizId) ? bizId : StringUtil.EMPTY);
				logs.add(log);
			});
	...
	public List<LogAccount> companyLogs() {
		...
		companyTips.entrySet().forEach(entry -> {
			entry.getValue().forEach(tips -> {
				...
				log.setAccountType(tips.type);
				log.setId(IDWorker.INSTANCE.nextSid());
				log.setOwnerType(TargetType.COMPANY);
				String bizId = StringUtil.hasText(tips.bizId) ? tips.bizId : this.bizId;
				log.setBizId(StringUtil.hasText(bizId) ? bizId : StringUtil.EMPTY);
				logs.add(log);
			});
